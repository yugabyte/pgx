...
func ParseConfigWithOptions(connString string, options ParseConfigOptions) (*ConnConfig, error) {
	...
	if s, ok := config.RuntimeParams["yb_servers_refresh_interval"]; ok {
		...
		if refresh, err := strconv.Atoi(s); err == nil {
			...
			return nil, pgconn.NewParseConfigError(connString, "invalid refresh_interval", err)
		}
	}
	...
	if s, ok := config.RuntimeParams["fallback_to_topology_keys_only"]; ok {
		...
		if b, err := strconv.ParseBool(s); err == nil {
			...
			return nil, pgconn.NewParseConfigError(connString, "invalid fallback_to_topology_keys_only", err)
		}
	}
